...
*/
int dc_module_hud_life_color(float block_fraction, float sine_value)
{
    int rgb_r = 0;
    int rgb_g = 0;
    int rgb_b = 0;
    ...
    */
    ...
    {
        rgb_r = DC_MODULE_HUD_RGB_HP_BLOCK_BLUE_R;
        rgb_g = DC_MODULE_HUD_RGB_HP_BLOCK_BLUE_G;
        rgb_b = DC_MODULE_HUD_RGB_HP_BLOCK_BLUE_B;
    }
    else if (block_fraction >= 0.50)
    {
        rgb_r = DC_MODULE_HUD_RGB_HP_BLOCK_YELLOW_R;
        rgb_g = DC_MODULE_HUD_RGB_HP_BLOCK_YELLOW_G;
        rgb_b = DC_MODULE_HUD_RGB_HP_BLOCK_YELLOW_B;
    }
    else if (block_fraction >= 0.25)
    {
        rgb_r = DC_MODULE_HUD_RGB_HP_BLOCK_AMBER_R;
        rgb_g = DC_MODULE_HUD_RGB_HP_BLOCK_AMBER_G;
        rgb_b = DC_MODULE_HUD_RGB_HP_BLOCK_AMBER_B;
    }
    else
    {
        rgb_r = DC_MODULE_HUD_RGB_HP_BLOCK_RED_R;
        rgb_g = DC_MODULE_HUD_RGB_HP_BLOCK_RED_G;
        rgb_b = DC_MODULE_HUD_RGB_HP_BLOCK_RED_B;
    }

    int rbg_composite = 0;
    int rgb_adjustment = DC_MODULE_HUD_RGB_HP_BLOCK_ADJUST_RANGE * sine_value;
...
*/
void dc_module_hud_draw_player_icon_frame(int player_index)
{   
    void entity = NULL();
    int border_sprite_index = getlocalvar(VAR_KEY_SPRITE_PLAYER_ICON_FRAME);
    int icon_sprite_index = 0;
    int border_pos_x = 0;
    int border_pos_y = DC_MODULE_HUD_PLAYER_ICON_POS_Y;
    int icon_pos_x = 0;
    int icon_pos_y = 0;
    char player_model = "";
    ...
    */
    border_pos_x = player_index * DC_MODULE_HUD_PLAYER_HUD_SIZE_X;
    border_pos_x += DC_MODULE_HUD_PLAYER_ICON_POS_X;
    ...
    */
    ...
    {
        icon_pos_x = border_pos_x + DC_MODULE_HUD_PLAYER_ICON_BORDER;
        icon_pos_y = border_pos_y + DC_MODULE_HUD_PLAYER_ICON_BORDER;

        drawsprite(icon_sprite_index, icon_pos_x, icon_pos_y, DC_MODULE_HUD_PLAYER_ICON_POS_Z);
    }
    
    /* Draw the border sprite. */
    if (border_sprite_index)
    {        
        drawsprite(border_sprite_index, border_pos_x, border_pos_y, DC_MODULE_HUD_PLAYER_ICON_POS_Z);
    }
}

void dc_module_hud_draw_background_active_player(int player_index)
{
    int sprite_index = getlocalvar(VAR_KEY_SPRITE_BACKGROUND_PLAYER_ACTIVE + player_index);
    int pos_x = player_index * DC_MODULE_HUD_PLAYER_HUD_SIZE_X;
    
    pos_x += DC_MODULE_HUD_BG_PLAYER_ACTIVE_POS_X;

    drawsprite(sprite_index, pos_x, DC_MODULE_HUD_BG_PLAYER_ACTIVE_POS_Y, DC_MODULE_HUD_BG_PLAYER_ACTIVE_POS_Z);
}

void dc_module_hud_draw_player_mp_meter(int player_index, void target, float sine_value)
{
    int i = 0;
    ...
    * the player HUD.
    */
    block_position_left = player_index * DC_MODULE_HUD_PLAYER_HUD_SIZE_X;
...
*/
void dc_module_hud_draw_player_hp_meter(int player_index, void target, float sine_value)
{
    /*
    * Health Meter
    *
    ...
    */
    ...
    health_fraction = DC_MODULE_HUD_PLAYER_HP_METER_BLOCK_MAX * get_health_fraction(target);

    /* Get Y position. */
    block_position_y = DC_MODULE_HUD_PLAYER_HP_METER_POS_Y;
    ...
    */
    block_space_x = block_size_x + DC_MODULE_HUD_PLAYER_HP_METER_BLOCK_MARGIN_LEFT + DC_MODULE_HUD_PLAYER_HP_METER_BLOCK_MARGIN_RIGHT;
    ...
    */
    block_position_left = player_index * DC_MODULE_HUD_PLAYER_HUD_SIZE_X;
    block_position_left += DC_MODULE_HUD_PLAYER_HP_METER_POS_X;
    ...
    for (i = 0; i < health_fraction; i++)
    {
        ...
        */
        ...
        color_overlay_rgb_color = dc_module_hud_life_color(block_fraction, sine_value);

        changedrawmethod(NULL(), "enabled", 1);
        changedrawmethod(NULL(), "tintmode", 1);
        changedrawmethod(NULL(), "tintcolor", color_overlay_rgb_color);

        drawsprite(sprite_index, block_position_x, block_position_y, DC_MODULE_HUD_PLAYER_HP_METER_POS_Z);
        ...
        * just there to give the meter some texture.
        ...
        color_overlay_size_x = block_size_x - DC_MODULE_HUD_PLAYER_HP_METER_BLOCK_BORDER * 2;
        color_overlay_size_y = block_size_y - DC_MODULE_HUD_PLAYER_HP_METER_BLOCK_BORDER * 2;
        color_overlay_pos_x = block_position_x + DC_MODULE_HUD_PLAYER_HP_METER_BLOCK_BORDER;
        color_overlay_pos_y = block_position_y + DC_MODULE_HUD_PLAYER_HP_METER_BLOCK_BORDER;
        color_overlay_pos_z = DC_MODULE_HUD_PLAYER_HP_METER_POS_Z + 1;
        color_overlay_rgb_color = dc_module_hud_life_color(block_fraction, sine_value);

        drawbox(color_overlay_pos_x, color_overlay_pos_y, color_overlay_size_x, color_overlay_size_y, color_overlay_pos_z, color_overlay_rgb_color, DC_MODULE_HUD_PLAYER_HP_METER_OVERLAY_ALPHA_MODE);
        */
    }
}
...
* 
*/
void dc_module_hud_player_join(int player_index)
{
    settextobj(7, 10, 100, 1, 999999994, "Name: " + getplayerproperty(player_index, "name"));

    dc_module_hud_draw_background_active_player(player_index);
    dc_module_hud_draw_player_icon_frame(player_index);
}


// Draw player HUD, with icons, magic jars, and
// life blocks for the target entity.
void dc_module_hud_player_master()
{
    ...
    for(player_index=0; player_index<max_players; player_index++)
    {
        ...
        */
        if(!target)
        {
            ...
            {
                dc_module_hud_player_join(player_index);
            }
        ...
        */
        ...
        dc_module_hud_draw_player_hp_meter(player_index, target, sine_value);        
    }
}
