...
void draw0001(void vEnt){
    ...
    /*
    draw0001 - http://www.caskeys.com/dc/?p=1314#draw0001
    Damon Vaughn Caskey
    2008_01_25
	~2011_03_29
	~2012_08_30
    ...
    2. This function will override other uses of drawmethod. Therefore values are taken and applied from entity variables stored
	...
	*/
    ...
    float	fFactor = 0.0;													//Autozoom calculation placeholder.
	float	fZ = 0.0;														//Current Z location.
	...
	*/
	...
	fScaleX		=	getentityvar(vEnt, IDXE_ADSCALEX);							//X scale adjustment.
    fScaleY		=	getentityvar(vEnt, IDXE_ADSCALEY);							//Y scale adjustment.
    iFlipX		=	getentityvar(vEnt, IDXE_ADFLIPX);							//Flip X.
    iFlipY		=	getentityvar(vEnt, IDXE_ADFLIPY);							//Flip Y.
    iShiftX		=	getentityvar(vEnt, IDXE_ADSHIFTX);							//Shift X.
    iFill		=	getentityvar(vEnt, IDXE_ADFILL);								//Fill.
    iRotate		=	getentityvar(vEnt, IDXE_ADROTATE);							//Rotate.
    iARotat		=	getentityvar(vEnt, IDXE_ADAROTAT);							//Auto Rotation.
    fMinZ		=	openborvariant("player_min_z");							//Minimum Z location.
    fMaxZ		=	openborvariant("player_max_z");							//Maximum Z location.
    fZ			=	getentityproperty(vEnt, "z");							//Ent's current Z location.

	if(CONF_DRAWMODE && (fZ > fMinZ || fZ < fMaxZ))								//Autozoom disabled and entity in bounds?
	{
		fFactor		= 1;													//Factor = 1 (100% normal size).
	}
	else
	{
		//Prevent 0 sum by making sure Z and min/maxz are always slightly different.
		if(fZ == fMinZ) ++fZ;
		if(fZ == fMaxZ) --fZ;
	...
	setentityvar(vEnt, IDXE_ADSCALER, fFactor);									//Store scale ratio for use by sub entity (shooting, spawning, binding etc.) functions.
	...
	*/
    if (iARotat)
    {
        ...
        setentityvar(vEnt, IDXE_ADROTATE, iRotate);
    }
