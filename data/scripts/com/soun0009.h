...
int soun0009(void vEnt, char cSam, int iPri, int iLVol, int iRVol, int iSpeed, int iLoop, int iTime, int B, int C)
{
    ...
    */
    ...
    if (iSnd != -1 && vEnt)											                                                //Sample and entity valid?
	{
        ...
        iSTime = getindexedvar(IDXG_SND_LSTT);
        iTest  = iETime - iSTime;
                     
        if (vEnt == getindexedvar(IDXG_SND_LSTE)  && (iTest < iTime))
        {                
            return -2;
        }
        ...
        setindexedvar(IDXG_SND_LSTI, iSnd);                                                                               //Store sample.
        setindexedvar(IDXG_SND_LSTT, openborvariant("elapsed_time"));                                                     //Store current time.
        setindexedvar(IDXG_SND_LSTE, vEnt);                                                                               //Store caller.
